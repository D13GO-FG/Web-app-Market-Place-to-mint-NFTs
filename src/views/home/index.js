...
const Home = () => {
	const [isMinting, setIsMining] = useState(false);
	// Estato de la imagen DNA, cambio dependiendo de la cuenta
	...
	const getPlatziPunksData = useCallback(async () => {
		if (platziPunks) {
			...
			const dnaPreview = await platziPunks.methods
				...
				.call();
			//Traer imagen de DNA del Smart Contract, llamada a la funcion
	...
	return (
		...
		>
			<Stack flex={1} spacing={{ base: 5, md: 10 }}>
				...
				<Text color={'gray.500'}>
					Platzi Punks es una colección de Avatares randomizados cuya metadata es
				...
				<Text color={'green.500'}>
					...
					previsualizador para averiguar cuál sería tu Platzi Punk si minteas en este
					momento
				</Text>
				<Stack
					spacing={{ base: 4, sm: 6 }}
			...
			>
				...
				<Text color={'green.600'}>Punks diponibles: {availablePunks}</Text>
				{active ? (
					<>
						<Flex mt={2}>
							<Badge>
